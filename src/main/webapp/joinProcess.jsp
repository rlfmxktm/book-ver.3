...
<%
   ...
   String url = "jdbc:mysql://localhost:3306/ebookshop"; //★드라이버 연결 설정-데이터연결 시 필요하므로 꼭 필요함(기본구문)
   ...
   try {
      ...
      String sql = "INSERT INTO member VALUES(?,?,?,?)"; //데이터 입력 sql 구문 
      ...
      response.sendRedirect("loginForm.jsp"); //회원가입 완료되면 loginForm.jsp로 이동
      
   } catch(Exception e) { //위의 조건이 실행되지 않을경우
      e.printStackTrace(); //오류 메세지 화면출력
      response.sendRedirect("joinForm.jsp"); //joinForm.jsp로 이동
   }   
   %>
